...
function compareNames(...args) {
  const sourceNames = []
  Array.from(args).forEach(source => sourceNames.push(
    source.systemMade
      ? source.userName
...
function compare2Sources(aId, bId) {
  ...
  const authUserId = store.getters['auth/user'].id
  ...
  if (aId === authUserId) {
    ...
    const trustedIds = store.getters['relatedSources/trustedIds']
    const followedIds = store.getters['relatedSources/followedIds']
...
function compareAssessments(a, b) {
  ...
  // anonymous requests for assessment do not have a SourceId
    ...
    for (const date of ['createdAt', 'updatedAt']) {
      const aDate = new Date(a.lastVersion[date])
      const bDate = new Date(b.lastVersion[date])
      if (aDate < bDate)
        return -1
      else if (aDate > bDate)
        return 1
    ...
    const compareVal = compare2Sources(a.assessor.SourceId, b.assessor.SourceId)
    if (compareVal !== 0) {
      return compareVal
    }
    else {
      const aCredVal = Math.abs(a.lastVersion.postCredibility)
      const bCredVal = Math.abs(b.lastVersion.postCredibility)
...
function compareSources(a, b) {
  const compareVal = compare2Sources(a.id, b.id)
  if (compareVal !== 0)
    return compareVal
  else
...
function compareBoosters(a, b) {
  const compareVal = compare2Sources(a.booster.id, b.booster.id)
  if (compareVal !== 0) {
    return compareVal
  }
  else {
...
function compareTitles(a, b) {
  ...
  const compareVal = compare2Sources(a.author.id, b.author.id)
  if (compareVal !== 0) {
    return compareVal
  }
  else {
    const aDate = new Date(a.lastVersion.updatedAt)
    const bDate = new Date(b.lastVersion.updatedAt)
    if (aDate < bDate)
      return 1
    else if (aDate > bDate)
      return -1
...
function extractHostname(url, removeProtocol) {
  ...
  if (url.includes('cnn.com/videos')) {
    const index = url.indexOf('/video/playlists/')
    if (index !== -1)
  ...
  if (url.includes('//')) {
    if (keepQueryParam) {
      ...
      const indexOfFirstParam = hostname.indexOf('?', hostname.indexOf('//') + 2)
...
async function followRedirects(link) {
  ...
  const regexIndex = twitterRegEx.exec(link) ? twitterRegEx.exec(link).index : false

  if (['dl.acm.org', 'kickstarter.com', 'linkedin.com'].some((siteURL) => {
    return link.includes(siteURL)
  }) || regexIndex) {
    return new Promise((resolve, reject) => resolve({ type: 'redirect chain not followed', link }))
  }
  else {
    let extractedURL
      ...
      */
      if (response.data && response.data.length <= 4000) {
        ...
        if (!window.location.hostname.includes('news.google.com')) {
          return ({ type: 'entire page is fetched', link })
        }
        else {
          /*
          ...
          const escapedHTML = escapeHTMLPolicy.createHTML(response.data)
      ...
      .catch((err) => {
        ...
        */
          ...
          return ({ type: 'error', detail: 'CORS', link })
        }
        else {
          // eslint-disable-next-line no-console
          console.log(err)
...
function constructAltURLs(urls) {
  const indexAlternativeUrls = urls.map((url) => {
  ...
  const extendedUrls = urls.concat(indexAlternativeUrls)
  return extendedUrls
}
