...
function getSiblings(e: Node) {
  // for collecting siblings
  const siblings: Array<Node> = []
...
function isEmbedded(link: Element) {
...
function populateLinkAssessments(allLinksAssessments) {
  for (const link of Object.keys(allLinksAssessments)) {
    if (allLinksAssessments[link].confirmed.length
      ...
      const linkEls = document.querySelectorAll(`a[href='${link}']`)
      console.log('did it find elements', linkEls, 'for link', link)

      if (linkEls.length) {
        const linkIsQuestioned = store.getters['linkAssessments/isQuestioned'](allLinksAssessments[link])
        if (linkIsQuestioned) {
          ...
          linkEls.forEach((linkEl) => {
            if (!linkEl.getAttribute('trustnet-modified-question-link')) {
              const iconToAddClone = questionIcon.cloneNode(true)
              let accuracyClass
        ...
        const linkIsRefuted = store.getters['linkAssessments/isRefuted'](allLinksAssessments[link])
        ...
        if (linkIsRefuted) {
          ...
          const linkIsConfirmed = store.getters['linkAssessments/isConfirmed'](allLinksAssessments[link])
          ...
          if (linkIsConfirmed) {
            ...
            const linkIsDebated = store.getters['linkAssessments/isDebated'](allLinksAssessments[link])
        ...
        if (specialStatus) {
          linkEls.forEach((linkEl) => {
            if (!linkEl.getAttribute('trustnet-modified-link')) {
              ...
              const iconToAddClone = iconToAdd.cloneNode(true)
              let accuracyClass
              ...
              browser.runtime.sendMessage({
                ...
                interaction: {
                  ...
                  data: {
                    ...
                    accuracyStatus: specialStatus || null,
