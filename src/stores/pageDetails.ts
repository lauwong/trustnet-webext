...
export default {
  ...
  actions: {
    ...
    setUpPageUrl(context) {
      return new Promise((resolve, reject) => {
        let sanitizedUrl

        const pageRawURL = window.location.href
    ...
    setBlackListStatus(context) {
      return new Promise((resolve, reject) => {
        const pageHostname = utils.extractHostname(context.state.url)
        let pageIsBlackListed = false

        let allBlackLists = constants.GLOBAL_BLACKLISTED_DOMAINS
        const userPreferences = context.rootState.preferences.userPreferences
